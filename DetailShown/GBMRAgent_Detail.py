...
class GMRAgent:
    ...
    def plotGmemory(self):
        ...
        for node in list(self.Gmemory.nodes()):
            pos[node] = (math.floor(self.Gmemory.nodes[node]['label']/10),self.Gmemory.nodes[node]['label']%10)
        ...
        for shell in self.reconstruct_paths:
            nx.draw_networkx(self.Gmemory,pos,nodelist=shell,with_labels=False,node_size=100,node_color='g')
