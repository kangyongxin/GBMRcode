...
class A3CAgent:
    ...
    def _init_op(self, model):
        state = tf.compat.v1.placeholder(tf.float32, shape=(None, state_size))
        action = tf.compat.v1.placeholder(tf.float32, shape=(None, action_size))
        # not immediate, but discounted n step reward
        q_target = tf.compat.v1.placeholder(tf.float32, shape=(None, 1)) 
        ...
        # Policy loss
        log_p = action * tf.math.log(tf.clip_by_value(policy,1e-10,1.))
        log_lik = log_p * tf.stop_gradient(td_error)
        actor_loss = -tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=log_lik, axis=1))

        # with tf.variable_scope('local_gradients'):
        # entropy(for more exploration)
        entropy = -tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=policy * tf.math.log(tf.clip_by_value(policy,1e-10,1.)), axis=1))
        # with tf.variable_scope('critic_loss'):
        # Value loss
        # critic_loss = tf.reduce_mean(tf.square(td_error))
        critic_loss = tf.reduce_mean(input_tensor=tf.square(value - q_target), axis=1)
        # Total loss
        ...
        train_op = tf.compat.v1.train.RMSPropOptimizer(self.learning_rate, decay=.99).minimize(loss_total)
