...
from envs.tpycolab import key_to_door
#from tensorflow.contrib import framework as contrib_framework
import tensorflow as tf
#nest = contrib_framework.nest
#tf.nest.framework.map_structure
...
class PycolabEnvironment(object):
  ...
  def __init__(self, game,
    ...
    #                             self._game.colours)
    colours = tf.nest.map_structure(lambda c: float(c) * 255 / 1000,self._game.colours)
  ...
  def trender(self,observation):
    new_im = Image.fromarray(observation)
    new_im.save('tempenv.png')
    print("not finished ")
