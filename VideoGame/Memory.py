...
class Memory:
    ...
    def node_feature_list(self):
        ...
        for nodeid in self.Gmemory.nodes():
            ...
            F.append(self.Gmemory.nodes[nodeid]['feature'])
    ...
    def _get_location(self,cur_state,next_state):
        if self.Gmemory.number_of_nodes() == 0:
            ...
            cur_similarity = self._write_content_similarity.onevsall(cur_state,self.node_feature_list())
            next_similarity = self._write_content_similarity.onevsall(next_state,self.node_feature_list())
        
            if max(cur_similarity)<self._TH:
                if max(next_similarity)<self._TH:
                    #print("add new node",max(similarity))
                    cur_node = self.Gmemory.number_of_nodes()
                    next_node = self.Gmemory.number_of_nodes()+1      
                else:
                    cur_node = self.Gmemory.number_of_nodes()
                ...
                if max(next_similarity)<self._TH:
                    ...
                    next_node = self.Gmemory.number_of_nodes()
                else:
                    cur_node = cur_similarity.index(max(cur_similarity))
                    next_node = next_similarity.index(max(next_similarity))
    ...
    def add_single_node(self,cur_node,cur_state,act,r):
        ...
        #self.StateAttributesDict.append(('cur_node':cur_state))
    ...
    def pairwriter(self,transition):
        [cur_state,act,r,next_state]=transition
        cur_node,next_node = self._get_location(cur_state,next_state)
        if cur_node == next_node:
            self.add_single_node(cur_node,cur_state,act,r)
        else:
            self.add_pair(cur_node,cur_state,act,r,next_node,next_state)